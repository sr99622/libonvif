...
usage()
{
  std::cout << "Usage: onvif-util [-ahsv] [-u <user>] [-p <password>] [command]"
            << std::endl;
}
...
showAll()
{
  struct OnvifSession* onvif_session =
    (struct OnvifSession*)calloc(sizeof(struct OnvifSession), 1);
  struct OnvifData* onvif_data =
    (struct OnvifData*)calloc(sizeof(struct OnvifData), 1);
  ...
  while (true) {
    ...
    if (strlen(element)) {
      ...
      memset(onvif_session->preferred_network_address,
             0,
             sizeof(onvif_session->preferred_network_address));
      strcpy(onvif_session->preferred_network_address, ip_address);

      int n = broadcast(onvif_session);
      std::cout << "Found " << n << " cameras on interface " << ip_address
                << std::endl;
      for (int i = 0; i < n; i++) {
...
showHelp()
{
  std::cout
    ...
    << "      -u  username\n"
    << "      -p  password\n"
    << "      -s  safe mode off, enable applications for viewer and browser to "
       "run\n\n"
    ...
    << "  Once logged into the camera you can view data using the 'get' "
       "command followed by the data requested.\n"
    << "  The (n) indicates an optional profile index to apply the setting, "
       "otherwise the current profile is used.\n\n"
    << "    Data Retrieval Commands (start with get)\n\n"
    << "      get rtsp 'pass'(optional) (n) - Get rtsp uri for camera, with "
       "optional password credential\n"
    << "      get snapshot 'pass'(optional) (n) - Get snapshot uri for camera, "
       "with optional password credential\n"
    << "      get capabilities\n"
    << "      get time\n"
    << "      get profiles\n"
    << "      get profile (n)\n"
    ...
    << "      set resolution (n) - Resolution setting in the format "
       "widthxheight, must match the video option\n"
    << "      set framerate (n)\n"
    << "      set gov_length (n)\n"
    << "      set bitrate (n)\n"
    ...
    << "      view (n) - View the camera output using ffplay (this assumes you "
       "have ffplay installed in the path\n"
    << "      view player (n) - View the camera output with user specified "
       "player e.g. view vlc\n"
    << "      dump - Full set of raw data from camera configuration\n"
    << "      sync_time 'zone'(optional) - Sync the camera time to the "
       "computer.  Optionally adjusts based on camera time zone\n"
    << "      reboot\n\n"
...
const std::string
cat(const char* arg1, const char* arg2)
{
...
std::string
uri_with_pass(OnvifData* onvif_data)
{
  std::string uri(onvif_data->stream_uri);
  std::stringstream ss;
  ss << uri.substr(0, 7) << onvif_data->username << ":" << onvif_data->password
     << "@" << uri.substr(7);
  return ss.str();
}

std::string
snapshot_with_pass(OnvifData* onvif_data)
{
...
void
profileCheck(OnvifData* onvif_data, const std::vector<std::string>& args)
{
  int index = 0;
  if (args.size() > 1) {
    ...
    if (getProfileToken(onvif_data, index))
      throw std::runtime_error(
        cat("get profile token - ", onvif_data->last_error));
    if (strlen(onvif_data->profileToken) == 0)
      throw std::runtime_error(
        cat("invalid profile token - ", (char*)std::to_string(index).c_str())
          .data());
  ...
  if (getProfile(onvif_data))
    throw std::runtime_error(cat("get profile - ", onvif_data->last_error));
}
...
main(int argc, char** argv)
{
  ...
  struct OnvifSession* onvif_session =
    (struct OnvifSession*)calloc(sizeof(struct OnvifSession), 1);
  struct OnvifData* onvif_data =
    (struct OnvifData*)malloc(sizeof(struct OnvifData));
  ...
  while (!found) {
    ...
    if (strlen(element)) {
      ...
      for (int i = 0; i < n; i++) {
        ...
        if (!success) {
          std::cout << "found invalid xaddrs in device response for device "
                    << i << std::endl;
          continue;
        }
        char host[128];
        ...
        if (!strcmp(host, wanted)) {
          ...
          if (getDeviceInformation(onvif_data) == 0) {
            ...
            // Initializing the session properly with the camera requires
            // calling getCapabilities
            if (getCapabilities(onvif_data)) {
              std::cout << "ERROR: get capabilities - "
            ...
            if (time_sync) {
              ...
              if (setSystemDateAndTime(onvif_data))
                throw std::runtime_error(
                  cat("set system date and time - ", onvif_data->last_error));
              std::cout << "  Camera date and time has been synchronized "
                           "without regard to camera timezone\n"
                        << std::endl;
              exit(0);
            }
            break;
          } else {
            std::cout << "ERROR: get device information - "
                      << onvif_data->last_error << "\n"
                      << std::endl;
            exit(1);
          }
      ...
      if (getDeviceInformation(onvif_data) == 0) {
        ...
        if (time_sync) {
          ...
          if (setSystemDateAndTime(onvif_data))
            throw std::runtime_error(
              cat("set system date and time - ", onvif_data->last_error));
          std::cout << "  Camera date and time has been synchronized without "
                       "regard to camera timezone\n"
                    << std::endl;
          exit(0);
        }
      } else {
      ...
      if (args[0] == "get") {
        ...
        if (args[0] == "rtsp") {
          ...
          if (getStreamUri(onvif_data))
            throw std::runtime_error(
              cat("get stream uri - ", onvif_data->last_error));
          std::string uri(onvif_data->stream_uri);
          if (add_pass) {
          ...
          profileCheck(onvif_data, args);
          if (getSnapshotUri(onvif_data))
            throw std::runtime_error(
              cat("get snapshot - ", onvif_data->last_error));
          std::string uri(onvif_data->stream_uri);
          if (add_pass) {
          ...
          if (getVideoEncoderConfiguration(onvif_data))
            throw std::runtime_error(cat("get video encoder configuration - ",
                                         onvif_data->last_error));
          ...
          if (getTimeOffset(onvif_data))
            throw std::runtime_error(
              cat("get time offset - ", onvif_data->last_error));
          std::cout << "  Time Offset: " << onvif_data->time_offset
                    << " seconds" << "\n";
          std::cout << "  Timezone:    " << onvif_data->timezone << "\n";
          std::cout << "  DST:         " << (onvif_data->dst ? "Yes" : "No")
                    << "\n";
          std::cout << "  Time Set By: "
                    << ((onvif_data->datetimetype == 'M') ? "Manual" : "NTP")
                    << "\n";
          if (onvif_data->datetimetype != 'M') {
            if (getNTP(onvif_data))
              throw std::runtime_error(
                cat("get NTP - ", onvif_data->last_error));
          ...
          if (args.size() > 1) {
            if (args[1] == "options") {
              ...
              for (int i = 0; i < size; i++) {
                std::cout << "    " << onvif_data->resolutions_buf[i]
                          << std::endl;
              }
              ...
              if (getVideoEncoderConfiguration(onvif_data))
                throw std::runtime_error(
                  cat("get video encoder configuration - ",
                      onvif_data->last_error));
              std::cout << "  Resolution: " << onvif_data->width << " x "
                        << onvif_data->height << "\n";
              std::cout << "  Frame Rate: " << onvif_data->frame_rate << "\n";
              std::cout << "  Gov Length: " << onvif_data->gov_length << "\n";
          ...
          if (args.size() > 1) {
            ...
            if (getImagingSettings(onvif_data))
              throw std::runtime_error(
                cat("get imaging settings - ", onvif_data->last_error));
          ...
          if (getNetworkInterfaces(onvif_data))
            throw std::runtime_error(
              cat("get network interfaces - ", onvif_data->last_error));
          if (getNetworkDefaultGateway(onvif_data))
            throw std::runtime_error(
              cat("get network default gateway - ", onvif_data->last_error));
          if (getDNS(onvif_data))
            throw std::runtime_error(cat("get DNS - ", onvif_data->last_error));
          ...
          profileCheck(onvif_data, args);
          if (getStatus(onvif_data))
            throw std::runtime_error(
              cat("get status - ", onvif_data->last_error));
        ...
        if (args[0] == "brightness") {
          if (args.size() > 1) {
            ...
            if (setImagingSettings(onvif_data))
              throw std::runtime_error(
                cat("set brightness - ", onvif_data->last_error));
          ...
          if (args.size() > 1) {
            ...
            if (setImagingSettings(onvif_data))
              throw std::runtime_error(
                cat("set contrast - ", onvif_data->last_error));
          ...
          if (args.size() > 1) {
            ...
            if (setImagingSettings(onvif_data))
              throw std::runtime_error(
                cat("set saturation - ", onvif_data->last_error));
          ...
          if (args.size() > 1) {
            ...
            if (setImagingSettings(onvif_data))
              throw std::runtime_error(
                cat("set sharpness - ", onvif_data->last_error));
          ...
          if (args.size() > 1) {
            ...
            if (found != std::string::npos) {
              ...
              if (setVideoEncoderConfiguration(onvif_data))
                throw std::runtime_error(
                  cat("set video encoder configuration - ",
                      onvif_data->last_error));
              std::cout << "  Resolution was set to " << onvif_data->width
                        << " x " << onvif_data->height << "\n"
                        << std::endl;
            } else {
              std::cout << "  Syntax error, proper format for the argument is "
          ...
          if (args.size() > 1) {
            ...
            if (setVideoEncoderConfiguration(onvif_data))
              throw std::runtime_error(cat("set video encoder configuration - ",
                                           onvif_data->last_error));
          ...
          if (args.size() > 1) {
            ...
            if (args[0] == "on") {
              ...
              if (onvif_data->dhcp_enabled) {
                ...
                if (setNetworkInterfaces(onvif_data))
                  throw std::runtime_error(
                    cat("set network interfaces - ", onvif_data->last_error));
                std::cout
                  << "  DHCP was enabled successfully\n\n"
          ...
          if (args.size() > 1) {
            ...
            if (getNetworkInterfaces(onvif_data))
              throw std::runtime_error(
                cat("get network interfaces - ", onvif_data->last_error));
            if (onvif_data->dhcp_enabled) {
              std::cout << "  Camera DHCP is enabled, IP address may not be "
                           "set manually\n"
                        << std::endl;
            } else {
              ...
              if (setNetworkInterfaces(onvif_data))
                throw std::runtime_error(
                  cat("set network interfaces - ", onvif_data->last_error));
              std::cout << "  IP Address has been changed, session will need "
                           "to be restarted\n"
                        << std::endl;
              exit(0);
            }
          ...
          if (args.size() > 1) {
            ...
            if (getNetworkDefaultGateway(onvif_data))
              throw std::runtime_error(
                cat("get network default gateway - ", onvif_data->last_error));
            if (onvif_data->dhcp_enabled) {
              std::cout << "  Camera DHCP is enabled, default gateway may not "
                           "be set manually\n"
                        << std::endl;
            } else {
              strcpy(onvif_data->default_gateway_buf, args[0].c_str());
              if (setNetworkDefaultGateway(onvif_data))
                throw std::runtime_error(
                  cat("set default gateway - ", onvif_data->last_error));
          ...
          if (args.size() > 1) {
            ...
            if (getDNS(onvif_data))
              throw std::runtime_error(
                cat("get DNS - ", onvif_data->last_error));
            if (onvif_data->dhcp_enabled) {
              std::cout
                << "  Camera DHCP is enabled, DNS may not be set manually\n"
                << std::endl;
            } else {
              strcpy(onvif_data->dns_buf, args[0].c_str());
              if (setDNS(onvif_data))
                throw std::runtime_error(
                  cat("set DNS - ", onvif_data->last_error));
          ...
          if (args.size() > 1) {
            ...
            if (setUser(buf, onvif_data))
              throw std::runtime_error(
                cat("set user - ", onvif_data->last_error));
        ...
        if (reply == "yes") {
          if (rebootCamera(onvif_data))
            throw std::runtime_error(
              cat("reboot camera - ", onvif_data->last_error));
        ...
        if (args.size() > 1) {
          ...
          if (args[0] == "zone") {
            profileCheck(onvif_data, args);
            if (setSystemDateAndTimeUsingTimezone(onvif_data))
              throw std::runtime_error(
                cat("set system date and time using timezone - ",
                    onvif_data->last_error));
            std::cout << "  Camera date and time has been synchronized using "
                         "the camera timezone\n"
                      << std::endl;
          }
        } else {
        ...
        safe_mode = false;
        std::cout << "  Safe mode has been turned off, run only known safe "
                     "apps and cameras\n"
                  << std::endl;
      } else if (args[0] == "view") {
        if (safe_mode) {
          std::cout << "  SAFE MODE ON, use 'unsafe' command to disable safe "
                       "mode, or -s option from command line\n"
                    << std::endl;
          continue;
        }
